...
import { Plus, Trash2, Settings, MessageSquare, Briefcase, Download, Upload, X, CheckSquare, Square, FolderSync, FolderCheck, Loader2 } from 'lucide-react';
...
const ConfigManager: React.FC<ConfigManagerProps> = ({ language, isOpen, onClose, onUpdate }) => {
  ...
  const handleLinkFolder = async () => {
      ...
      try {
          ...
          console.error(e);
          // Only show alert if it's not a user cancellation or iframe block (which throws visible error)
          if (e.name !== 'AbortError' && !e.message.includes('blocked')) {
             alert(t.syncError);
          }
      } finally {
          setIsLinking(false);
  ...
  const handleExportConfig = () => {
    // If synced, overwrite directly
    if (fileSystemService.isHandleReady()) {
        configService.syncToDisk().then(success => {
      ...
      <div className="bg-white w-full max-w-5xl h-[85vh] rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="bg-gray-900 text-white p-4 flex justify-between items-center shrink-0">
           <div className="flex items-center gap-3">
             ...
             <h2 className="text-xl font-bold">{t.configManagerTitle}</h2>
           </div>
           ...
           <div className="flex gap-2 items-center">
                 ...
                 <button 
                    ...
                    disabled={isLinking || isIframe}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded text-xs font-medium transition-colors border ${
                        isSynced 
                        ? 'bg-green-900/50 border-green-700 text-green-200 hover:bg-green-900' 
                        : isIframe 
                            ? 'bg-gray-100 border-gray-200 text-gray-400 cursor-not-allowed'
                            : 'bg-gray-800 border-gray-600 text-gray-300 hover:bg-gray-700 hover:text-white'
                    }`}
                    title={isIframe ? "Not available in preview mode (Open in new tab)" : (isSynced ? t.folderSynced : t.linkFolder)}
                 >
                    {isLinking ? <Loader2 className="w-4 h-4 animate-spin" /> : isSynced ? <FolderCheck className="w-4 h-4 text-green-400" /> : <FolderSync className="w-4 h-4" />}
                    <span className="hidden sm:inline">{isSynced ? t.folderSynced : t.linkFolder}</span>
                 </button>
               ...
               <div>
                ...
                {editTemplate ? (
                   <div className="bg-gray-50 p-6 rounded-xl border border-blue-100 space-y-4">
                      ...
                      <div>
                         <label className="text-xs font-bold text-gray-500 mb-1 block">{t.lblDesc}</label>
                         <textarea className="w-full p-2.5 border rounded-lg bg-white text-sm h-16 resize-none" placeholder={t.lblDesc} value={editTemplate.description || ''} onChange={e => setEditTemplate({...editTemplate, description: e.target.value})} />
                      </div>
                      <div>
